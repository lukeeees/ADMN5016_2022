...
def show_predict_page():
    ...
    m_dep = col1.slider("Mobile Depth (cm)", 0.1,1.0,0.2) #8
    mobile_wt = col1.slider("Mobile Weight", 80,200,100) #9
    n_cores = col1.slider("Core Processor",1,8,2) #10
    pc = col1.slider("Back Camera",0,20,10) #11
    px_height = col1.slider("Pixel Resolution height",0,1960,1000) #12 
    px_width = col1.slider("Pixel Resolution width",500,1998,1000) #13
    ram = col2.slider("RAM",256,3998,1256)    #14
    sc_height = col2.slider("Screen height",5,19,12)  #15
    sc_width = col2.slider("Screen width",0,18,5) #16
    talk_time = col2.slider("Longest time that a single battery charge will last",2,20,11) #17
    touch_screen = col2.selectbox("Touch Screen", criteria) #18
    wifi = col2.selectbox("Wifi", criteria)
